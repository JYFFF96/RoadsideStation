...
def main():
 ...
 print("RoadsideStation V0.6.10.1 Discovery Diagnostics starting...")
 ...
 print("Qt/C++ portability: rescue/discovery/quality/diagnostic logic uses scalar point/track evidence only; no CARLA actor data.")
 ...
 print("CARLA roadside sensors started: %d"%len(station.sensors));print("V0.6.10.1 CARLA evaluator: %s"%("enabled" if evaluator else "disabled"))
 ...
 print("ARCH: V0.6.10.1 Discovery Diagnostics observes source stages and discovery-born track lifecycle only.")
 ...
 try:
  while not _STOP_REQUESTED:
   ...
   if now-last>=1.0:
    ...
    _print_sparse_geometry(s);_print_discovery_diagnostics(dds);_print_detection_stability(ds)
   ...
   if evaluator is not None and now-last_eval>=eval_interval:
    ...
    _print_sparse_geometry(s);_print_discovery_diagnostics(dds);_print_detection_stability(ds);_print_detection_drop(dd)
