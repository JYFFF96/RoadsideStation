...
def _print_road_object_stage_attribution(report):
 for item in report.get("actors",[]) or []:
  ...
  print("    [ROAD-OBJECT STAGE ACTOR] id=%d type=%s range=%.1f..%.1fm Visible:%d Raw:%d(%s pts_avg/max=%.1f/%d) Component:%d Shape:%d Temporal:%d DedupePass:%d Baseline:%d Balanced:%d"%(item.get("actor_id",0),item.get("type_id","unknown"),float(item.get("range_min",0.0) or 0.0),float(item.get("range_max",0.0) or 0.0),visible,raw,_stage_rate(raw,visible),raw_avg,item.get("raw_points_max",0),stages.get("component",0),stages.get("shape",0),stages.get("temporal",0),stages.get("dedupe_pass",0),stages.get("output",0),stages.get("balanced_output",0)))
 for band in report.get("range_bands",[]) or []:
  visible=band.get("visible_frames",0)
  print("    [ROAD-OBJECT RANGE STAGE %.0f-%.0fm] Actors:%d Visible:%d Raw:%d(%s) Component:%d(%s) Shape:%d(%s) Temporal:%d(%s) DedupePass:%d(%s) Baseline:%d(%s) Balanced:%d(%s)"%(band.get("min_range",0.0),band.get("max_range",0.0),band.get("actors",0),visible,band.get("raw_frames",0),_stage_rate(band.get("raw_frames",0),visible),band.get("component",0),_stage_rate(band.get("component",0),visible),band.get("shape",0),_stage_rate(band.get("shape",0),visible),band.get("temporal",0),_stage_rate(band.get("temporal",0),visible),band.get("dedupe_pass",0),_stage_rate(band.get("dedupe_pass",0),visible),band.get("output",0),_stage_rate(band.get("output",0),visible),band.get("balanced_output",0),_stage_rate(band.get("balanced_output",0),visible)))
...
def main():
 ...
 print("RoadsideStation V0.6.12.8.2.2.3 Range-Balanced Recovery Cap Shadow starting...")
