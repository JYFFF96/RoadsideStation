...
def main():
 ...
 parser.add_argument("--rsu-rsi-id",default=None,
                     help="CSAE RoadSideInformation id: exactly 8 ASCII bytes")
 ...
 if args.rsu_rsi_id is not None:
  config.setdefault("dachuan_rsu",{})["rsi_id"]=args.rsu_rsi_id
 ...
 print("RoadsideStation V0.6.12.8.2.2.88 Complete RSI Frame starting...")
 ...
 print("Camera Ground Initiation: %s | range=%.0f..%.0fm bottom-center ray/ground plane | class-aware ROI diagnostics, Shadow only"%(
 ...
 print("Camera Temporal Admission: %s | frames>=%d match<=%.1fm VRU extra=%.1fm | Shadow only"%(
  "enabled" if fc.get("camera_ground_temporal_shadow_enabled",False) else "disabled",
 ...
 if camera_source=="detector" and config.get("camera_detection",{}).get("enabled",True):
  try:
   camera_detector=create_camera_detector(config.get("camera_detection",{}));print("Camera detector active in fusion loop: %s model=%s confidence=%.2f nms=%.2f"%(camera_detector.name,config.get("camera_detection",{}).get("model","-"),float(config.get("camera_detection",{}).get("confidence",.35)),float(config.get("camera_detection",{}).get("nms",.45))))
 ...
 try:
  while not _STOP_REQUESTED:
   ...
   camera_ground_candidates=camera_ground_shadow.update(
    ...
    frame_token=camera_ground_token)
   if evaluator is not None:
    evaluator.observe_camera_ground_initiation(
     camera_ground_candidates,frame_id=camera_ground_token)
    evaluator.observe_camera_ground_vru_roi_ablation(
     camera_ground_shadow.last_vru_roi_ablation_candidates,
   ...
   if args.verbose and now-last>=1.0:
    ...
    camera_vru_roi=evaluator.report_camera_ground_vru_roi_ablation() if evaluator is not None else {}
    for margin,value in sorted(camera_vru_roi.items(),key=lambda item:float(item[0])):
    ...
    for gate,value in sorted((camera_enforced.get("identity_gates",{}) or {}).items(),
    ...
    for gate,value in sorted((camera_enforced.get("reassociation_gates",{}) or {}).items(),
                             key=lambda item:float(item[0])):
    ...
    tombstone_feature=camera_enforced.get("tombstone_feature",{}) or {}
    feature_name="%s_%s"%(tombstone_feature.get("mode","-"),
                           _num(tombstone_feature.get("gate")))
    for label,value in sorted((tombstone_feature.get("labels",{}) or {}).items()):
     heading=value.get("heading_cos",{}) or {};speed=value.get("tombstone_speed",{}) or {}
    ...
    if camera_detector is not None:
     ...
     print("      [CAMERA DETECTOR RUNTIME] Name:%s Runtime:%s Frames:%d Detections:%d AvgLatency:%.1fms MaxLatency:%.1fms Classes:%s"%(
      detector_report.get("name","-"),detector_report.get("runtime","-"),detector_report.get("frames",0),detector_report.get("detections",0),detector_report.get("latency_ms_avg",0.0),detector_report.get("latency_ms_max",0.0),detector_report.get("classes",{})))
