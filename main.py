...
def _print_selected_track_admission_profile(report):
 ...
 if camera_verdict.get("enabled",False):
  ...
  print("    [SELECTED CAMERA RESCUE DEPLOYMENT VERDICT] Status:%s Rule:%s Samples:%d/%d P:%s/>=%s FPReject:%s/>=%s ActorCoverage:%s/>=%s ConfirmFPReject:%s/>=%s Sources:%s Required:%s Reasons:%s | EVALUATION-ONLY"%(
...
def main():
 ...
 print("RoadsideStation V0.6.12.8.2.2.42 Sensor Timestamp Skew Shadow starting...")
 ...
 try:
  while not _STOP_REQUESTED:
   ...
   if now-last>=1.0:
    ...
    print("  [SENSOR SYNC SHADOW] Camera-LiDAR FrameDelta:%s TimeDelta:%s | latest-frame observation only"%(("-" if sync_frames is None else "%+d"%sync_frames),("-" if sync_seconds is None else "%+.3fs"%sync_seconds)))
