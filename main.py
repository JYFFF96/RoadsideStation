...
def main():
 ...
 print("RoadsideStation V0.6.4 Far-range Stability + Track Persistence starting...")
 ...
 print("Track persistence: coast_frames=%d confidence_decay=%.2f max_age=%.1fs"%(int(fc.get("track_coast_frames",0)),float(fc.get("track_coast_confidence_decay",.88)),float(fc.get("track_max_age",1.5))))
 ...
 print("CARLA roadside sensors started: %d"%len(station.sensors));print("V0.6.4 CARLA evaluator: %s"%("enabled" if evaluator else "disabled"))
 ...
 print("ARCH: traffic -> ground removal -> road ROI -> distance-aware far score -> track persistence -> fusion")
  ...
  while not _STOP_REQUESTED:
   ...
   if now-last>=1.0:
    ...
    for idx,o in enumerate(fol.objects[:10]):
     t=fusion.last_tracked_candidates[idx];size=o.size;rs="-" if o.radar_speed is None else "%.2f"%o.radar_speed;cam="-" if o.camera is None else "%s box=%s"%(o.camera.get("cameraId","?"),o.camera.get("bbox"));near=t.get("radar_nearest_xy");near_txt="-" if near is None else "%.2f"%near;raw_speed=math.hypot(t.get("raw_vx",0),t.get("raw_vy",0));fused_speed=math.hypot(o.vx,o.vy);state=t.get("track_state","update")
     print("  %-12s type=%-7s state=%-6s pos=(%7.2f,%7.2f,%5.2f) vel=(%6.2f,%6.2f) speed=%.2f raw=%.2f size=(%.2f,%.2f,%.2f) radar=%s near=%sm hits=%d cam=%s conf=%.2f src=%s"%(o.object_id,o.object_type,state,o.x,o.y,o.z,o.vx,o.vy,fused_speed,raw_speed,size[0],size[1],size[2],rs,near_txt,int(t.get("radar_hits",0)),cam,o.confidence,"+".join(o.sources)))
    last=now
   if evaluator is not None and now-last_eval>=eval_interval:
    ...
    print("  [TRACK LIFE] NEW:%d UPDATE:%d COAST:%d DROP:%d"%(s.get("track_new",0),s.get("track_update",0),s.get("track_coast",0),s.get("track_drop",0)))
