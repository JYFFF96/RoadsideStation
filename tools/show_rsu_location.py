...
def resolve_station(world_map, cfg):
    ...
    if deployment in ("auto_junction", "auto_cross_junction"):
        ...
        base = carla.Transform(carla.Location(x=x, y=y, z=z),
                               carla.Rotation(yaw=sensor_yaw))
        return base, carla.Location(x=center.x, y=center.y, z=center.z), junction.id, dirs, area
    t = sc["transform"]
    base = carla.Transform(
        carla.Location(x=float(t.get("x", 0)), y=float(t.get("y", 0)), z=float(t.get("z", 8))),
        carla.Rotation(pitch=float(t.get("pitch", 0)), yaw=float(t.get("yaw", 0)), roll=float(t.get("roll", 0))))
...
def forward_endpoint(transform, distance):
    ...
    return carla.Location(
        x=transform.location.x + distance * cp * math.cos(yaw),
        y=transform.location.y + distance * cp * math.sin(yaw),
        z=transform.location.z + distance * math.sin(pitch))
...
def main():
    ...
    ap.add_argument("--seconds", type=float, default=60.0,
                    help="debug marker lifetime; spectator remains at the RSU afterwards")
    ap.add_argument("--height", type=float, default=55.0,
                    help="spectator height above junction")
    args = ap.parse_args()
    ...
    if junction_id is not None:
        print("Junction: id=%s directions=%d area=%.1f center=(%.2f, %.2f)" %
              (junction_id, dirs, area, center.x, center.y))
    print("RSU_001: x=%.2f y=%.2f z=%.2f yaw=%.1f" %
          (base.location.x, base.location.y, base.location.z, base.rotation.yaw))
    print("Camera yaw=%.1f pitch=%.1f | Radar yaw=%.1f pitch=%.1f | LiDAR=360deg" %
          (camera_t.rotation.yaw, camera_t.rotation.pitch,
           radar_t.rotation.yaw, radar_t.rotation.pitch))

    # Put spectator above and slightly behind the RSU, looking at the junction.
    ...
    world.get_spectator().set_transform(
        carla.Transform(carla.Location(x=sx, y=sy, z=sz),
    ...
    # Junction center and RSU pole.
    dbg.draw_point(ground_center, size=0.45, color=carla.Color(255, 255, 0), life_time=life)
    dbg.draw_string(carla.Location(x=center.x, y=center.y, z=center.z + 2.0),
                    "JUNCTION CENTER", draw_shadow=True, color=carla.Color(255, 255, 0),
                    life_time=life, persistent_lines=False)
    dbg.draw_line(carla.Location(x=base.location.x, y=base.location.y, z=center.z),
                  rsu_mark, thickness=0.12, color=carla.Color(255, 0, 255), life_time=life)
    dbg.draw_point(rsu_mark, size=0.55, color=carla.Color(255, 0, 255), life_time=life)
    dbg.draw_string(carla.Location(x=rsu_mark.x, y=rsu_mark.y, z=rsu_mark.z + 1.0),
                    "RSU_001", draw_shadow=True, color=carla.Color(255, 0, 255),
                    life_time=life, persistent_lines=False)

    # Camera direction: green. Radar direction: red. LiDAR: cyan cross/ring-like spokes.
    cam_end = forward_endpoint(camera_t, 35.0)
    radar_end = forward_endpoint(radar_t, 35.0)
    dbg.draw_line(camera_t.location, cam_end, thickness=0.15,
                  color=carla.Color(0, 255, 0), life_time=life)
    dbg.draw_string(cam_end, "CAMERA ->", draw_shadow=True,
                    color=carla.Color(0, 255, 0), life_time=life, persistent_lines=False)
    dbg.draw_line(radar_t.location, radar_end, thickness=0.15,
                  color=carla.Color(255, 0, 0), life_time=life)
    dbg.draw_string(radar_end, "RADAR ->", draw_shadow=True,
                    color=carla.Color(255, 0, 0), life_time=life, persistent_lines=False)
    for deg in range(0, 360, 30):
        r = math.radians(deg)
        end = carla.Location(x=lidar_t.location.x + 8.0 * math.cos(r),
                             y=lidar_t.location.y + 8.0 * math.sin(r),
                             z=lidar_t.location.z)
        dbg.draw_line(lidar_t.location, end, thickness=0.04,
                      color=carla.Color(0, 255, 255), life_time=life)
    dbg.draw_string(carla.Location(x=lidar_t.location.x, y=lidar_t.location.y,
                                   z=lidar_t.location.z + 1.8),
                    "LiDAR 360", draw_shadow=True, color=carla.Color(0, 255, 255),
                    life_time=life, persistent_lines=False)
