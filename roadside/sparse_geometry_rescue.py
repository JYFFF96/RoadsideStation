...
def _empty_band_stats():
    return {"eligible": 0, "quality_block": 0, "streak_block": 0,
...
def track_guided_sparse_rescue(points, previous_tracks, world_transform,
    ...
    """Portable sparse supplement chain.

    V0.6.11.1: range-aware track-guided rescue.
    V0.6.11.2: current-frame 50-80m Far Geometry Builder before NewDiscovery.
    Ground truth is never consumed; all outputs still pass normal ROI/Score/Track.
    """
    c = config or {}
    out = []
    ...
    if c.get("sparse_geometry_rescue_enabled", False) and points is not None and previous_tracks:
        ...
        for track in previous_tracks or []:
            ...
            rng = _range_xy(lx, ly)
            if rng < min_range or rng > max_range or _near_existing(lx, ly, occupied, dedupe):
                continue
            ...
            _add_total(diag, "eligible"); band["eligible"] += 1
            quality = float(track.get("track_quality", 0.0))
            if quality < min_quality:
                _add_total(diag, "quality_block"); band["quality_block"] += 1
                continue
            previous_streak = _previous_rescue_streak(track)
            if previous_streak >= max_streak:
                _add_total(diag, "streak_block"); band["streak_block"] += 1
                continue
            ...
            support = []
            for p in pts:
                dx = float(p[0]) - lx; dy = float(p[1]) - ly
                if dx * dx + dy * dy <= r2 and abs(float(p[2]) - lz) <= z_window:
                    support.append(p)
            if len(support) < need:
                _add_total(diag, "support_block"); band["support_block"] += 1
                continue
            e = _extent(support)
            hl = max(float(e[0]), float(e[1])); hs = min(float(e[0]), float(e[1])); h = float(e[2])
            if hl < min_length or hl > max_length or hs < min_width or hs > max_width or h < min_height or h > max_height:
                _add_total(diag, "geometry_block"); band["geometry_block"] += 1
                continue
            streak = previous_streak + 1
            n = float(len(support))
            item = {"x": sum(float(p[0]) for p in support) / n,
                    "y": sum(float(p[1]) for p in support) / n,
                    "z": sum(float(p[2]) for p in support) / n,
                    "point_count": len(support), "extent": e,
                    "cluster_mode": "sparse_rescue", "scale_votes": 1,
                    "scale_modes": ["sparse_rescue", "rescue_streak_%d" % streak],
                    "sparse_rescued": True, "sparse_discovered": False,
                    "rescue_track_id": track.get("id"),
                    "rescue_track_hits": int(track.get("track_hits", 0)),
                    "rescue_range": rng, "rescue_gate_band": band_name,
                    "rescue_gate_quality": quality,
                    "rescue_gate_min_quality": min_quality,
                    "rescue_gate_max_streak": max_streak}
            out.append(item); occupied.append(item)
            _add_total(diag, "built"); band["built"] += 1

    # V0.6.11.2: independent current-frame far geometry builder.
    far_built = build_far_geometry_candidates(points, occupied, c)
    for item in far_built:
        x = dict(item)
        out.append(x)
        occupied.append(x)

    # V0.6.10 NewDiscovery remains unchanged and dedupes against FarBuilder.
