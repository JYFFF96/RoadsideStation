...
class NearestTracker(object):
    def __init__(self,max_distance=4.0,max_age=1.5,max_speed=20.0,velocity_alpha=.35,extent_alpha=.25,extent_shrink_alpha=.05,extent_lock_hits=5,radar_velocity_alpha=.35,velocity_window=5,position_alpha=.45,stationary_speed=.35,coast_frames=0,coast_confidence_decay=.88):
        ...
        self.coast_frames=max(0,int(coast_frames));self.coast_confidence_decay=float(coast_confidence_decay)
        self._tracks={};self._next_id=1;self.last_stats={"new":0,"update":0,"coast":0,"drop":0}
    ...
    def _coast_item(self,tid,t,now):
        dt=max(0.0,now-float(t.get("timestamp",now)));item=dict(t.get("last_det",{}))
        item.update({"id":tid,"x":float(t["x"])+float(t["vx"])*dt,"y":float(t["y"])+float(t["vy"])*dt,"z":float(t["z"]),"raw_x":float(t["x"]),"raw_y":float(t["y"]),"raw_z":float(t["z"]),"raw_vx":float(t["vx"]),"raw_vy":float(t["vy"]),"vx":float(t["vx"]),"vy":float(t["vy"]),"extent":list(t.get("extent",[0,0,0])),"track_hits":int(t.get("hits",1)),"track_state":"coast","coast_frames":int(t.get("misses",0))})
    ...
    def update(self,detections,timestamp=None):
        now=time.time() if timestamp is None else float(timestamp);unmatched=set(self._tracks);results=[];pending=[];stats={"new":0,"update":0,"coast":0,"drop":0}
        ...
        for di,det in enumerate(detections):
            ...
            item=dict(det);item.update({"id":tid,"x":x,"y":y,"z":z,"raw_x":raw_x,"raw_y":raw_y,"raw_z":raw_z,"raw_vx":raw_vx,"raw_vy":raw_vy,"track_vx_before_radar":pre_radar_vx,"track_vy_before_radar":pre_radar_vy,"vx":vx,"vy":vy,"extent":extent,"track_hits":hits,"track_state":"new" if old is None else "update","coast_frames":0});results.append(item)
        # Missed detections remain eligible for reassociation until max_age. For a
        # short configurable window they are also emitted as predicted/coasting
        # tracks so ObjectList does not blink on a one-frame LiDAR miss.
        for tid in list(unmatched-assigned_track):
            t=self._tracks.get(tid)
            if t is None:continue
            t["misses"]=int(t.get("misses",0))+1
            if now-float(t.get("timestamp",now))<=self.max_age and t["misses"]<=self.coast_frames:
