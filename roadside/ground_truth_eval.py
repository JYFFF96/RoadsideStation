...
class GroundTruthEvaluator(object):
    ...
    def _parse_bins(self, values):
        ...
        for value in values or []:
            try:
                ...
                continue
            if value > 0.0 and value <= self.radius and value not in bins:
                bins.append(value)
        bins.sort()
        if not bins or abs(bins[-1] - self.radius) > 1e-6:
            bins.append(self.radius)
        return bins
    ...
    def truth_vehicles(self):
        ...
        for actor in self.world.get_actors().filter("vehicle.*"):
            try:
                ...
                distance = self._distance2d(loc, center)
    ...
    def _detected_with_range(self, detected):
        ...
        for det in detected or []:
            item = dict(det)
            item["range"] = math.hypot(float(item.get("x", 0.0)) - float(center.x),
                                       float(item.get("y", 0.0)) - float(center.y))
            if item["range"] <= self.radius:
                out.append(item)
        return out
    ...
    def _metrics(truth_n, detected_n, pairs):
        ...
        return {
            ...
            "max_position_error": max(errors) if errors else None,
        }
    ...
    def _range_metrics(self, truth, detected):
        ...
        for upper in self.range_bins:
            gt_bin = [x for x in truth if lower <= x.get("range", 0.0) < upper or
                      (upper == self.range_bins[-1] and lower <= x.get("range", 0.0) <= upper)]
            det_bin = [x for x in detected if lower <= x.get("range", 0.0) < upper or
                       (upper == self.range_bins[-1] and lower <= x.get("range", 0.0) <= upper)]
            pairs = self._match(gt_bin, det_bin)
            metrics = self._metrics(len(gt_bin), len(det_bin), pairs)
            metrics["min_range"] = lower
            metrics["max_range"] = upper
    ...
    def evaluate(self, detected_tracks, camera_objects=None, camera_pairs=None, radar_matched=0):
        ...
        detected = self._detected_with_range(detected_tracks)
        pairs = self._match(truth, detected)
        metrics = self._metrics(len(truth), len(detected), pairs)
        metrics.update({
            ...
            "truth_objects": truth,
            "pairs": pairs,
            "range_bins": self._range_metrics(truth, detected),
        })
        return metrics
